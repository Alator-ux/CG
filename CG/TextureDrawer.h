...
public:
    ...
    void draw(std::vector<HighLevelInterface> &scene, Camera &camera, bool textured)
    {
        clear();
        /*for (auto& prim : highlvl_obj.objects) {
        ...
        z_buffer(scene, camera, textured);
    }
    ...
    void draw(std::vector<RayTraceObjectInterface *> &scene, glm::vec3 cameraPos, RLight light, int fov)
    {
        ...
        fov /= 2;
        size_t need_to_color = width * height;
        auto tfov = glm::tan(glm::radians(fov * 1.f)) * width / height;
        for (auto x = 0; x < width; x++)
        {
            for (auto y = 0; y < width; y++)
            {
                glm::vec3 ray = {
                    (2 * (x + 0.5) / width - 1) * tfov * width / height,
                    -(2 * (y + 0.5) / height - 1) * tfov,
                    1,
                };
                auto tracer = RayTracer(scene, light, ray);
                Rnormalize(ray);
